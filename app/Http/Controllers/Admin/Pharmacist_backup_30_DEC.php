...
class Pharmacist extends Controller
{
   ...
   private function tenantSetUp($host_name, $data) {
    ...
    public  function  update_validity(Request $request)
    {
      ...
      return redirect('admin/all_pharmacies')->with('msg','<div class="alert alert-success""> Subcription   <strong> plan validity </strong> extends.</div>');
   ...
   public  function  get_connection($website_id)
    ...
    {   
        ...
        if($request->role=='technician' && $getAccess->no_of_technicians <= count($getAllTechnician) ){ 
            ...
            return redirect()->back()->withInput()->with('msg','<div class="alert alert-danger""> your pharmacy can create only number of '.$getAccess->no_of_technicians.' Technician </div>');
        }
